...
- **Nginx Pitfalls**: Nginx-based containers (e.g., Dashboards, Proxies) often require the `CHOWN` and `SETUID` capabilities during startup to initialize `/var/cache/nginx`. If the pod crashes with a `chown failed` error, add these capabilities and ensure `emptyDir` volumes are mounted to `/var/cache/nginx` and `/var/run`.
- **User-Switching Pitfalls**: Some containers (e.g., Tautulli) start as root to perform initialization and then switch to a non-privileged user. These containers REQUIRE the `SETUID` and `SETGID` capabilities. If the pod fails with an `operation not permitted` error during a user switch, add these capabilities to the `securityContext`.
